...
def main(event, context):
    ...
    download_from_s3(BUCKET_NAME, 'today_fb_data.csv', '/tmp/today_fb_data.csv')

    # Load data into dataframes
    iso_data = pd.read_csv('/tmp/today_iso_data.csv')
    hh_data = pd.read_csv('/tmp/today_hh_data.csv')
    fb_data = pd.read_csv('/tmp/today_fb_data.csv')
    ...
    ## Merge fb_data on 'batter' and 'date'
    fb_data.drop(columns=['team', 'batter_id'], inplace=True)
    merged_data = pd.merge(merged_data, fb_data, on=['batter', 'date'], how='inner')
    ...
    merged_data['iso'] = merged_data['iso'].apply(lambda x: 1 if x >= 200 else 0)
    merged_data['hard_hit'] = merged_data['hard_hit'].apply(lambda x: 1 if x >= 10 else 0)
    merged_data['fb'] = merged_data['fb'].apply(lambda x: 1 if x >= 100 else 0)
    # Drop unnecessary columns
    ...
    features = ['iso', 'hard_hit', 'fb', 'pitcher_hand', 'batter_id', 'pitcher_id']
