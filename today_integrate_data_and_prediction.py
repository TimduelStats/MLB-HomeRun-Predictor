import pandas as pd
import player_map
# Sample data frames (fb_data, hh_data, pitcher_matchup_data, onlyhomers_data)
# Load them from CSVs if necessary
fb_data = pd.read_csv('today_fb_data.csv')
hh_data = pd.read_csv('today_hh_data.csv')
pitcher_matchup_data = pd.read_csv('pitcher_matchup_data.csv')


# Merge fb_data and hh_data on 'batter' and 'date'
merged_data = pd.merge(fb_data, hh_data, on=['batter', 'date'], how='inner')

# ------------------------------------------------------------

# Merge based on date and game_date
merged_data = pd.merge(merged_data, pitcher_matchup_data,
                       left_on='date', right_on='game_date', how='left')
...
# The apply method now returns both the pitcher name and pitcher hand as a pd.Series,
# which is split into two columns (pitcher and pitcher_hand) in the merged_data DataFrame.
merged_data[['pitcher', 'pitcher_hand']] = merged_data.apply(
    lambda row: pd.Series(map_opponent(row)), axis=1)


# Drop unnecessary columns from pitcher_matchup_data
merged_data = merged_data.drop(columns=['game_date', 'team_away', 'team_home',
                               'pitcher_away_hand', 'pitcher_away', 'pitcher_home', 'pitcher_home_hand', 'matchup_id'])

# ------------------------------------------------------------


# Drop the temporary 'pitcher_new' column
# With inplace=True: The original merged_data DataFrame is modified directly, so you don't need to reassign it.
merged_data.drop(
    columns=['date', 'batter_id'], inplace=True)

# ------------------------------------------------------------

merged_data['batter_id'] = merged_data['batter'].map(
    player_map.batter).fillna(-1).astype(int)

# ------------------------------------------------------------
# ------------------------------------------------------------
# Create pitcher id for each batter

merged_data['pitcher_id'] = merged_data['pitcher'].map(
    player_map.pitcher).fillna(-1).astype(int)

# Drop all the rows where batter_id or pitcher_id is -1
merged_data = merged_data[merged_data['batter_id'] != -1]
merged_data = merged_data[merged_data['pitcher_id'] != -1]

# ------------------------------------------------------------
# make RHP to 1, LHP to 0
merged_data['pitcher_hand'] = merged_data['pitcher_hand'].apply(
    lambda x: 1 if x == 'RHP' else 0)

merged_data.drop(columns=['batter', 'team', 'pitcher'], inplace=True)

# turn hh and fb to binary
merged_data['fb'] = merged_data['fb'].apply(
    lambda x: 1 if x >= 100 else 0)
merged_data['hard_hit'] = merged_data['hard_hit'].apply(
    lambda x: 1 if x >= 10 else 0)

# # Save the final data to a CSV file
merged_data.to_csv('today_final_combined_data.csv', index=False)

print("Data combined and saved successfully!")

# Make prediction
import joblib
features = ['fb', 'hard_hit', 'pitcher_hand', 'batter_id', 'pitcher_id']  # Example of feature columns
...
today_data = pd.read_csv('today_final_combined_data.csv')
xgb_model_loaded = joblib.load('xgb_model1.pkl')
predictions = xgb_model_loaded.predict(today_data[features])
